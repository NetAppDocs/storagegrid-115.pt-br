...
NOTE: Dependendo da sua versão do vCenter, a ordem das etapas variará para aceitar o Contrato de Licença de Usuário final, especificando o nome da máquina virtual e selecionando um datastore

. Selecione armazenamento para a máquina virtual.
+

NOTE: Se você estiver implantando um nó como parte da operação de recuperação, execute as instruções no <<step_recovery_storage,etapa de recuperação de armazenamento>> para adicionar novos discos virtuais, reconete discos rígidos virtuais do nó de grade com falha ou ambos.
...
NOTE: A localização do *vApp Options* depende da versão do vCenter.
