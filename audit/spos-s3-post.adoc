...
summary: Quando um cliente S3 emite uma solicitação de restauração PÓS-objeto, é feita uma solicitação para restaurar um objeto do armazenamento do AWS Glacier para um Cloud Storage Pool. Esta mensagem é emitida pelo servidor se a transação for bem-sucedida. 
---
= SPOS: S3 POST
:allow-uri-read: 
:icons: font
